 ...
 *   - Bloque 6: BeanOutputConverter + Bean Validation post-parseo
...
public class CodeReviewService {
    ...
    // BeanOutputConverter genera el JSON Schema del record Review y parsea la respuesta.
    // Se instancia una vez porque es stateless (el schema no cambia).
    private final BeanOutputConverter<Review> converter = new BeanOutputConverter<>(Review.class);

    public CodeReviewService(ChatClient chat, Validator validator) {
        this.chat = chat;
    ...
    public Review review(String language, String code) {
        ...
        try {
            ...
            // Metadata para el log de observabilidad (criterio de éxito 2 y 3)
            ...
            // Parseo el texto de respuesta al record Review usando el converter
            ...
            log.warn("LLM transient error — status={} latency_ms={} — will retry", status, latencyMs);
